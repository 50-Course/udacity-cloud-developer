...
(async () => {
  ...
  app.get('/cars', async (req: Request, res: Response) => {
    ...
    if ( make ) {
      ...
      // filter out the specific car from the showroom
      let car = cars.filter((car: Car) => car.make === make);
  ...
  app.post('/cars/:id', async (req: Request, res: Response) => {
    let { id } = req.params;

    if ( !id ) {
      return res.status(400)
                .send('Car  \'id\' must be provided');
    }
